...
The folder **cubifySvalbard** contains results and a link to the documentation of a side project where the ICESat-2 elevation changes were rasterized. The folder **data** contains data used as input for running this code (training data). The folder **results** contains results that this thesis produced (geojson of the classified glaciers with the according probability). The folder **svalbardsurges** contains scripts for the detection of surges in Svalbard.
