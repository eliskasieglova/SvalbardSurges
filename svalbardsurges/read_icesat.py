...
def readICESat(icesat_products):
    ...
    outpath = Path('data/ICESat.csv')

    # cache
    if outpath.is_file():
        return pl.read_csv(outpath).to_pandas()

    # read products
    for icesat_product in icesat_products:
        if Path(f'data/{icesat_product}.csv').exists():
            continue
...
def readATL06():
    path = Path('data/raw/ATL06')
    outpath = Path(f'data/ATL06.csv')
    ...
    if outpath.exists():
        data = pl.read_csv(outpath).to_pandas()
        ...
        for filepath in os.listdir(path):
            ...
            with h5py.File(path/filepath, 'r') as file:
                ...
                for beam in beams:
                    ...
                    temp_beam["QS"] = list(file[beam]['land_ice_segments']['atl06_quality_summary'])
                    temp_beam["delta_time"] = list(file[beam]['land_ice_segments']['delta_time'])
                    temp_beam["dem_h_icesat"] = list(file[beam]['land_ice_segments']['dem']['dem_h'])
...
def readATL08():
    path = Path('data/raw/ATL08')
    outpath = Path(f'data/ATL08.csv')
...
def readATL03():
    path = Path('data/raw/ATL03')
    outpath = Path('data/ATL03.csv')
...
def readATL08QL():
    path = Path('data/raw/ATL08QL')
    outpath = Path(f'data/ATL08QL.csv')
